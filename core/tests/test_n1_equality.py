...
class FinanceOpenInvoicesTermScopeTest(TestCase):
    """
    Regression: open_invoices must be scoped to the current term, not global.

    The original bug was a global Invoice.objects.exclude(PAID).count() that
    counted unpaid invoices from ALL terms. The fix changed it to
    term_invoices.exclude(PAID).count() where term_invoices is filtered to the
    current term. This test creates unpaid invoices in a DIFFERENT term and
    verifies the dashboard's open_invoices does NOT include them.
    """
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="fin_officer", password="pass123",
            email="fin@test.com", role="finance_officer",
        )
        self.ay = AcademicYear.objects.create(
            name="2026", start_date=date(2026, 1, 1), end_date=date(2026, 12, 31),
        )
        # Term dates must span today so the view's date-based resolution picks this term
        today = _date.today()
        self.term = Term.objects.create(
            academic_year=self.ay, name="Term 1",
        ...
        self.other_term = Term.objects.create(
            academic_year=self.ay, name="Term 2",
        ...
        self.student = Student.objects.create(
            ...
            admission_no="ADM-REG", class_name="Grade 1",
            academic_year=self.ay,
        )

        # Create 3 UNPAID invoices in the current term
        for i in range(3):
            Invoice.objects.create(
                student=self.student, amount_due=Decimal("1000"),
                status=InvoiceStatus.UNPAID, term=self.term,
            )
        # Create 5 UNPAID invoices in a DIFFERENT term — these must NOT be counted
        for i in range(5):
            Invoice.objects.create(
                student=self.student, amount_due=Decimal("2000"),
                status=InvoiceStatus.UNPAID, term=self.other_term,
            )
        # Create 1 PAID invoice in current term — must NOT be counted
        Invoice.objects.create(
            student=self.student, amount_due=Decimal("500"),
            status=InvoiceStatus.PAID, term=self.term,
        )

    def test_open_invoices_matches_billing_term_scope(self):
        """
        Dashboard open_invoices must equal term-scoped count, not global count.

        If this fails, open_invoices has reverted to a global
        Invoice.objects.exclude(PAID).count() — the exact bug from the
        original finding. The fix is at finance/views.py line 246:
            open_invoices = term_invoices.exclude(status=InvoiceStatus.PAID).count()
        where term_invoices is Invoice.objects.filter(term_q) scoped to the
        current term.
        """
        ...
        # Dashboard-reported open_invoices from rendered HTML
        import re
        html = resp.content.decode("utf-8")
        open_m = re.search(r"(\d+)\s*accounts", html)
        dashboard_open = int(open_m.group(1)) if open_m else -1

        # Raw reference: term-scoped only
        term_invoices = Invoice.objects.filter(term=self.term)
        ref_open = term_invoices.exclude(status=InvoiceStatus.PAID).count()

        # Global count — this is what the BUG would have produced
        global_open = Invoice.objects.exclude(status=InvoiceStatus.PAID).count()

        self.assertEqual(
            dashboard_open, ref_open,
            f"open_invoices must be term-scoped: dashboard={dashboard_open}, "
            f"term_scoped={ref_open}, GLOBAL={global_open} (BUG)",
        )
        self.assertNotEqual(
            dashboard_open, global_open,
            f"open_invoices={dashboard_open} equals global count {global_open} — "
            f"the term-scoping fix is NOT applied",
        )

    def test_billed_received_also_term_scoped(self):
        """total_billed and total_received must match term-scoped computation."""
        from django.test import Client
        from finance.models import Invoice, Payment, InvoiceStatus, PaymentMethod
        from django.db.models import Sum

        # Add a payment to one current-term invoice
        inv = Invoice.objects.filter(term=self.term).first()
        Payment.objects.create(
            invoice=inv, amount=Decimal("500"), method=PaymentMethod.CASH,
            created_by=self.user, payment_date=date.today(),
        )
        ...
        import re
        html = resp.content.decode("utf-8")
        billed_m = re.search(r"color:#023AA5[^>]*>(\d+)<", html)
        received_m = re.search(r"color:#059669[^>]*>(\d+)<", html)
        dashboard_billed = int(billed_m.group(1)) if billed_m else -1
        dashboard_received = int(received_m.group(1)) if received_m else -1

        term_invoices = Invoice.objects.filter(term=self.term)
        ref_billed = term_invoices.aggregate(t=Sum("total_due"))["t"] or 0
        ref_received = Payment.objects.filter(
            invoice__in=term_invoices, is_reversal=False
        ).aggregate(t=Sum("amount"))["t"] or 0

        self.assertEqual(
            dashboard_billed, ref_billed,
            f"total_billed: dashboard={dashboard_billed}, ref={ref_billed}",
        )
        self.assertEqual(
            dashboard_received, ref_received,
            f"total_received: dashboard={dashboard_received}, ref={ref_received}",
        )
